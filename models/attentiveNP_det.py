...
class AttentiveNP_det(nn.Module):
    def __init__(self, args):
        super(AttentiveNP_det, self).__init__()
        ...
        self.attn = Attn()
        self.dec = Decoder(self.x_size, self.h_size)
    ...
    def forward(self, trajs, times, training=True):
        if training:
            ...
            x_mu, x_sigma = self.dec(times, attn_vec)

            return x_mu, x_sigma, torch.Tensor([0]), torch.Tensor([1]), torch.Tensor([0]), torch.Tensor([1])
        else:
            value_for_attn = self.det_enc(times[:25], trajs[:, :25, :])  # BS x traj len x Hidden Dim
...
class SelfAttn(nn.Module):
    def __init__(self, x_dim, h_dim):
        ...
        self.q_weight = nn.Linear(x_dim + h_dim, 2 * (x_dim + h_dim), bias=False)
        self.k_weight = nn.Linear(x_dim + h_dim, 2 * (x_dim + h_dim), bias=False)
        self.v_weight = nn.Linear(x_dim + h_dim, 2 * (x_dim + h_dim), bias=False)
        self.attn = Attn()
...
class Decoder(nn.Module):
    def __init__(self, x_size, h_size):
        ...
        self.self_attn = SelfAttn(self.x_size, self.h_size)

        self.decoder = nn.Sequential(nn.Linear(2 * (1 + self.h_size), 40),
                                     nn.ReLU(),
                                     nn.Linear(40, 40),
                                     nn.ReLU(),
                                     nn.Linear(40, 40),
    ...
    def forward(self, times, h_vector):
        ...
        self_attn_input = torch.cat((times, h_vector), dim=-1)  # BS x TL x (1 + H + H)
